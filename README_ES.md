...
- **Ataques de inclusión de ficheros locales:** Incluir dentro de la pagina web un fichero local, sitios de plantillas o descargas expuestos. Por ejemplo, cuando se pasa por url el fichero que se quiere utilizar para algo. Si no se acota a lo que puede acceder el servidor web, se puede acceder a archivos del propio Linux o Windows donde esta alojada la pagina. Se puede utilizar `php://filter/convert.base64-encode/resource=PATH_TO_FILE` para poder extraer ficheros con la extensión `php` sin que se ejecuten
